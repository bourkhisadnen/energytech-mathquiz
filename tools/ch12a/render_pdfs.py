"""A PDF twin of every figure, for the worksheet export.

The app shows SVG, which is what a browser wants. pdflatex cannot read SVG at
all, so the worksheet -- which is LaTeX -- needs the same drawings as PDF. They
come from the same compile, so the two can never be different pictures: the page
is lifted out of the multi-page PDF whole, without re-rendering.
"""
import os, re, json, shutil, subprocess, hashlib
from mkfig import SRC, preamble, fig_macros
from render_figs import macro_body

OUT = '/tmp/energytech_app/ch12new/figpdf'
WORK = '/tmp/energytech_app/ch12new/figwork_pdf'
