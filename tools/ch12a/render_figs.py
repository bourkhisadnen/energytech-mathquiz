...
OUT = '/tmp/energytech_app/ch12new/figsvg'
WORK = '/tmp/energytech_app/ch12new/figwork'
...
def macro_body(pre, name):
    ...
    for j in range(i, len(pre)):
        if pre[j] == '{' and pre[j-1] != '\\': depth += 1
        elif pre[j] == '}' and pre[j-1] != '\\':
            depth -= 1
            if depth == 0: return pre[i+1:j]
    raise ValueError(name)

if __name__ == '__main__':
    # Collect every figure body, deduplicated by content.
    bodies = {}          # sha -> body
    ...
    # One document per version's preamble is wrong (macros may call version-local
    # helpers), so compile per version but only the figures that version defines.
    order = []
    for v, path in SRC.items():
        pre = preamble(path)
        mine = [(n, s) for n, s in usage[v].items() if not os.path.exists(f'{OUT}/{s}.svg')]
        mine = list({s: n for n, s in mine}.items())          # one page per distinct sha
    ...
    json.dump(usage, open('/tmp/energytech_app/ch12new/fig_usage.json', 'w'), indent=1)
